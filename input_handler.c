...
int joystickSelect(int joystick_x, int joystick_y) {
    ...
    if (abs(delta_x) > abs(delta_y)) {
        if (delta_x < -DEADZONE) return 0; // LEFT -> Choice 0
        if (delta_x > DEADZONE) return 1;  // RIGHT -> Choice 1
    } else {
        if (delta_y < -DEADZONE) return 2; // UP -> Choice 2
        if (delta_y > DEADZONE) return 3;  // DOWN -> Choice 3
    }
